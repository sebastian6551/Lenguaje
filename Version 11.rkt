...
(define lexica
  '(
    ...
    (cadena ("\"" (arbno (or letter whitespace digit)) "\"")  string)
    )
  )
...
(define eval-expresiones
  (lambda (exp env) 
    (cases expresion exp
      ...
      (cadena-lit (cadena) (substring cadena 1 (- (string-length cadena) 1)))
      (id-lit (id) (re-apply-env env id))
      (id-ref (id) (apply-env-ref env id))
      ...
      (modificar-exp (id exp)
               (let ((once-eval (apply-env env id)))
                 ...
                 (if (list? once-eval)
                     (cond
                       ...
                       [(eqv? (car once-eval) 'unic)
                        (if (eqv? (car (cdr once-eval)) "C-VID-VAL")
                            (setref! (apply-env-ref env id) (list 'unic (eval-expresiones exp env)))
                            (eopl:error 'modificar-exp "No es una variable mutable, ~s ya ha sido asignada" id))]
                     ...
                     ;; Paso por referencia
                     (if (reference? once-eval)
                         (let ((once-eval-deref (deref once-eval)))
                           
                           (if (list? once-eval-deref)
                           (cond

                             ;;Para unic
                             [(eqv? (car once-eval-deref) 'unic)
                              (if (eqv? (car (cdr once-eval-deref)) "C-VID-VAL")
                                  (setref! once-eval (list 'unic (eval-expresiones exp env)))
                                  (eopl:error 'modificar-exp "No es una variable mutable, la referencia de ~s ya ha sido asignada" id))]

                             ;;Para var
                             [(eqv? (car once-eval-deref) 'var) (setref! once-eval (list 'var (eval-expresiones exp env)))]
    ...
    (cases procedimiento proc
      (closure (ids body env)
               (if (eqv? (contar-lista args) (contar-lista ids))
                   (eval-expresiones body (extend-env ids args env))
                   (eopl:error 'apply-procedimiento "El número de argumentos esperado: ~s, no coincide con el número dado: ~s" (contar-lista ids) (contar-lista args))
                   )))))
...
(scan&parse "
global ()
rec longitudLista(L){if compare L == vacio
                    then 0

  ...
  (lambda (prog)
    (cases programa prog
      (un-programa (globals exps)
                ...
                ; (eval-expresiones exps (init-env))
                 ))))
...
(define eval-globales
 ...
 (let ((args (eval-rands bodies env)))
             (eval-expresiones exps (extend-env-for-asign idss args env)
   )))
  ...
  (lambda (exp env) 
    (cases expresion exp
      ...
      (var-exp (ids exps body)
               (let ((args (eval-rands exps env)))
                 (eval-expresiones body (extend-env-for-asign ids args env))))
   ...
   ;; (reg-exp (id exp ids exps) id exp ids exps)
     ...
     ;; Asignación de variables
      (modificar-exp (id exp)
                     (begin (setref! (apply-env-ref env id) (eval-expresiones exp env)) ))
...
(define apply-env
  (lambda (env sym)
    ...
    (deref(cases environmentSet env
      (empty-env-rec ()
                        (eopl:error 'apply-env-ref "No binding for ~s" sym))
      (extended-env-rec (syms vals env)
